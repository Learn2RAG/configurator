...
vector_size = 1024
encoder = HuggingFaceEmbeddings(model_name='BAAI/bge-m3', encode_kwargs={'normalize_embeddings': True})
qdrant = Qdrant(collection_name, encoder, vector_size)


def recall(search_results, labels):
  count = 0
  for (label, search_result) in zip(labels, [[hit for hit in result] for result in search_results]):
    print(label)
    print(search_result)
...
search_results = [[Qdrant.vector_store.similarity_search(query, k=4)[i].metadata['source'].split('/')[-1].split('.pdf')[0] for i in range(4)]
